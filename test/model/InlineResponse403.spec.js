/*
 * MyOhioAssembly.com Public API
 * REST API for MyOhioAssembly.com
 *
 * OpenAPI spec version: 0.1.0
 * Contact: contact@myohioassembly.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 3.0.46
 *
 * Do not edit the class manually.
 *
 */
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', '../../src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require('../../src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.MyOhioAssemblycomPublicApi);
  }
}(this, function(expect, MyOhioAssemblycomPublicApi) {
  'use strict';

  var instance;

  describe('(package)', function() {
    describe('InlineResponse403', function() {
      beforeEach(function() {
        instance = new MyOhioAssemblycomPublicApi.InlineResponse403();
      });

      it('should create an instance of InlineResponse403', function() {
        // TODO: update the code to test InlineResponse403
        expect(instance).to.be.a(MyOhioAssemblycomPublicApi.InlineResponse403);
      });

      it('should have the property err (base name: "err")', function() {
        // TODO: update the code to test the property err
        expect(instance).to.have.property('err');
        // expect(instance.err).to.be(expectedValueLiteral);
      });

      it('should have the property errCode (base name: "errCode")', function() {
        // TODO: update the code to test the property errCode
        expect(instance).to.have.property('errCode');
        // expect(instance.errCode).to.be(expectedValueLiteral);
      });

    });
  });

}));
